...
<?php define('MEMBER_ID', $_SESSION['memberID'])?>
	...
	<div class="h3">Trash</div>

	<?php foreach ($db->query("SELECT * FROM mail WHERE reciver_id =".MEMBER_ID." AND mail_status = 'trash' order by id desc") as $row){?>
				<?php $member_details = hablu_gate_user_info($db, $row['sender_id']);?>
				<!-- single inbox mail -->
				<a href="single.php?mid=<?php echo $row['id'];?>&ref=trash" class="msg">
